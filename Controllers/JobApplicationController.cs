...
namespace SEP.Controllers
{
	...
	public class JobApplicationController : Controller
	{
		...
		public JobApplicationController(ApplicationDbContext db, UserManager<ApplicationUser> userManager, INotyfService notyfService)
		{
			_db = db;
		...
		public async Task<IActionResult> UploadDocument(List<IFormFile> files, string description, Guid id)
		{
			...
			foreach (var file in files)
			{
				...
				{
					_toastNotification.Error("Invalid file type. You can only upload .png, .jpg and .pdf files.");
				...
				if (!System.IO.File.Exists(filePath))
				{
					...
					_db.SaveChanges();
					_toastNotification.Success("File successfully uploaded.");

				}
				else
				{
					_toastNotification.Error("File has already been uploaded.");
				}
			}
		...
		public async Task<IActionResult> DeleteFile(Guid id)
		{
			...
			_db.SaveChanges();
			_toastNotification.Success($"Removed {file.Name + file.Extension} successfully.");
		...
		public async Task<IActionResult> SubmitApplication(Guid id)
		{
			...
			if (documents.Count > 0)
			{
				...
				_db.SaveChanges();
				_toastNotification.Success("Application successfully completed.");
				return RedirectToAction("FilteredJobPosts", "Student");
			}
			else
			{
				_toastNotification.Error("You cannot submit an empty application. Please upload the necessary documents.");
		...
		public async Task<IActionResult> UpdateApplicationStatus(Guid id, ApplicantDetailsViewModel vm)
		{
			...
			if (status != application.Status)
			{
				...
				_db.SaveChanges();
				_toastNotification.Success("Application status updated");
			}
